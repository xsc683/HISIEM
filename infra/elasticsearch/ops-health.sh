...
#   2. 索引大小排行:siem-events-* 按天递增、无 restored_* 残留
...
echo
echo "    判读:siem-events-* 应按天递增;restored_* 为演练残留(应清理)"
...
echo "==> 4) ILM 推进(siem-events-*/siem-alerts)"
for idx in "siem-events-*" "siem-alerts"; do
  echo "    -- $idx --"
...
echo "==> 5) 数据量快照"
events=$(curl -s "$ES/siem-events-*/_count" | grep -o '"count":[0-9]*' | cut -d: -f2)
alerts=$(curl -s "$ES/siem-alerts/_count" 2>/dev/null | grep -o '"count":[0-9]*' | cut -d: -f2)
echo "    siem-events-*: $events 事件"
