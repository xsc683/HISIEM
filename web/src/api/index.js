...
// SOAR
export const listSoarPlaybooks = (resourceType) => request(`/soar/playbooks${resourceType ? `?${new URLSearchParams({ resourceType })}` : ''}`)
export const reloadSoarPlaybooks = () => request('/soar/playbooks/reload', { method: 'POST' })
export const listSoarExecutions = (size = 50) => request(`/soar/executions?${new URLSearchParams({ size })}`)
export const getSoarExecution = (id) => request(`/soar/executions/${segment(id)}`)
export const getSoarExecutionEvents = (id) => request(`/soar/executions/${segment(id)}/events`)
export const listSoarAutomationRules = () => request('/soar/automation-rules')
export const scanSoarAutomationRules = () => request('/soar/automation-rules/scan', { method: 'POST' })
export const listSoarConnectors = () => request('/soar/connectors')
export const getSoarConnectorRuntime = () => request('/soar/connectors/runtime')
export const listSoarRevisions = (state) => request(`/soar/designer/revisions${state ? `?${new URLSearchParams({ state })}` : ''}`)
export const createSoarDraft = (definition, layout) => request('/soar/designer/drafts', json('POST', { definition, layout }))
export const updateSoarDraft = (playbookId, revision, definition, layout, lockVersion) => request(`/soar/designer/${segment(playbookId)}/revisions/${revision}`, json('PUT', { definition, layout, lockVersion }))
export const submitSoarRevision = (playbookId, revision) => request(`/soar/designer/${segment(playbookId)}/revisions/${revision}/submit`, { method: 'POST' })
export const reviewSoarRevision = (playbookId, revision, approved, note) => request(`/soar/designer/${segment(playbookId)}/revisions/${revision}/review`, json('POST', { approved, note }))
export const publishSoarRevision = (playbookId, revision, rolloutPercentage) => request(`/soar/designer/${segment(playbookId)}/revisions/${revision}/publish`, json('POST', { rolloutPercentage }))
export const importSoarGit = () => request('/soar/designer/import-git', { method: 'POST' })
export const startSoarExecution = (playbookId, resourceType, resourceId) => request('/soar/executions', json('POST', { playbookId, resourceType, resourceId }))
export const decideSoarApproval = (id, approved) => request(`/soar/executions/${segment(id)}/approval`, json('POST', { approved }))
export const retrySoarExecution = (id) => request(`/soar/executions/${segment(id)}/retry`, { method: 'POST' })
export const cancelSoarExecution = (id) => request(`/soar/executions/${segment(id)}/cancel`, { method: 'POST' })
export const pauseSoarExecution = (id) => request(`/soar/executions/${segment(id)}/pause`, { method: 'POST' })
export const resumeSoarExecution = (id) => request(`/soar/executions/${segment(id)}/resume`, { method: 'POST' })
