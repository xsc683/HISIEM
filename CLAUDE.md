...
2. **Mapper 扫描**：每个应用只扫自己需要的 mapper —— `HsiemPlatformApplication` 三条 `@MapperScan` 分别扫 detection（`detectionSqlSessionFactory`）、control 与 tenant（`controlPlaneSqlSessionFactory`）、soar（`soarSqlSessionFactory`）；`SoarWorkerApplication` 只扫 control 与 soar；`DetectionControllerApplication` 用显式白名单 `@MapperScan(basePackageClasses = {ManagedDetectionMapper, DetectionArtifactMapper, DetectionRuntimeMapper, DetectionControllerMapper})`（control-api 只依赖 detection-control，不扫 detection-runtime/artifact mapper，保证 control-api↔detection-runtime 边界封闭）。control/soar 的扫描都带 `annotationClass = Mapper.class` 且显式 `sqlSessionFactoryRef`，避免 starter 的 `@ConditionalOnMissingBean` 模板误绑。新加 mapper 时必须同步加进对应应用的扫描范围。
