...
**场景举例(单节点副本陷阱)**:副本必须分配到**不同于主分片所在的节点**;单节点下副本无处可放,于是 ①索引状态为 yellow,②每次写入要写两份(写入放大)。

**一般技术原则**:单节点环境通常把 `number_of_replicas` 设为 `0`,以消除 yellow 与写入放大。

**本项目的实际选择**:本项目 5 个索引模板实际配置的是 `number_of_replicas: 1`(见 `infra/elasticsearch/` 各模板)。这是**刻意的工程取舍**——`infra/elasticsearch/apply-templates.sh` 明确写着「单节点开发环境会显示 yellow;生产至少两节点」:即**宁可让单节点开发环境显示 yellow,也要让模板在生产形态(≥2 节点)下开箱可用**,避免开发与生产各维护一套配置。代价就是本地单节点会看到 yellow。

> **两者的区别**:「单节点应设 0」是**一般技术原则**;「本项目设 1」是**当前项目的既定取舍**。项目事实以仓库配置为准,不要按一般原则去改它。
...
**本项目应用**:事件索引按天创建,并已配置 ILM 策略(`infra/elasticsearch/apply-templates.sh` 创建 4 条 policy,`index.lifecycle.name` 写在对应的索引模板里):`hot`(近期热数据,可检索)→ `delete`(超过保留期自动删除),实现自动留存管理。当前事件索引保留 365 天、告警/案件 180 天、解析失败 raw 索引 30 天。

**覆盖范围要说准**:`infra/elasticsearch/` 下 6 个模板里,**4 个**带 `index.lifecycle.name`(`siem-events`、`siem-events-raw`、`siem-alerts`、`siem-cases`);`siem-entity-risk` 与 `asset-criticality` **没有保留策略**——它们不随时间删除,这是刻意的(实体风险是累积画像、资产关键性是小字典表),不是遗漏。
...
1. **副本数:本项目配的是 1,而不是单节点惯例的 0**:一般原则是单节点设 0 以消除 yellow 与写入放大;本项目**刻意**保留 1(见 `infra/elasticsearch/apply-templates.sh` 的说明「单节点开发环境会显示 yellow;生产至少两节点」),使模板在生产形态下直接可用。代价是本地会出现 yellow。
...
2. 单节点环境**一般**把副本设为多少?本项目**实际**配的是多少、为什么?(一般原则是 0;本项目实际为 1,属刻意取舍——见 §3.4)
