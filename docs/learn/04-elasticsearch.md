...
**本项目应用**:事件索引按天创建,并已配置 ILM 策略(见 `infra/elasticsearch/apply-templates.sh` 与各索引模板的 `index.lifecycle.name`):`hot`(近期热数据,可检索)→ `delete`(超过保留期自动删除),实现自动留存管理。当前事件索引保留 365 天、告警/案件 180 天、解析失败 raw 索引 30 天。
