...
**定位边界**:Kafka 是**短期缓冲 / 事件总线**,**长期存储归 Elasticsearch**。因此 Kafka 的保留期(retention)配置得较短(3 天,见 `infra/kafka/create-topics.sh` 的 `RETENTION_MS`),而 ES 通过 ILM 承担长期留存。
...
| **partition(分区)** | 主题在物理上的切片;每条消息按 offset 顺序写入其中一个分区 | 已配置 3 分区(`infra/kafka/create-topics.sh`) |
...
| **retention(保留期)** | 消息在 Kafka 中的保留时长 | 已配置 3 天(`create-topics.sh` 的 `RETENTION_MS`) |
...
**场景举例**:若 `siem-events` 只有 1 个分区,组 `siem-detection` 无论分配多少并行度,实际消费只有一个线程在运行;将分区增加到 3 个后,该组最多可启用 3 个线程并行消费(若组内只有 1 个消费者,则串行读全部 3 个分区)。这也是本项目将分区数从 1 增至 3 的原因:分区数只能增加、不能减少,应尽早规划,故 `create-topics.sh` 直接以 3 分区创建、并对已有 topic 自动扩容。
