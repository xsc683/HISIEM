...
| Managed Detection Runtime | Phase 4 observed-state foundation 已落地：V17 持久化 job group、assignment、expected/observed manifest 和 per-rule runtime status；desired API 保持 `PENDING`，仅显式 observe 可收敛状态 | [模块边界](design/module-boundaries.md) |
...
| 自动化验证 | 根 Maven reactor 208 个测试、Flink 独立工程 38 个测试、前端 9 个单元测试、生产构建和 3 条 Playwright E2E 通过；包含真实 PostgreSQL V17、日志查询约束/竞态、运营聚合、fencing/续租、并行/循环、Flink DLQ 与离开保存 | [路线图](roadmap.md) |
...
- Managed Detection Runtime 的 placement、Job Assignment、canonical Runtime Manifest 和 RuleRuntimeStatus 已作为 observed-state foundation 持久化；当前不执行物理 Flink/Docker controller，下一阶段才负责 apply/observe 调度。- Kafka、Flink、Logstash 的健康探针区分“真正健康”和“仅端口可达”，避免把降级结果误报为完整健康。
