...
阶段 4.1 已将控制面从文件/内存逐步迁移到 PostgreSQL：用户、角色、案件处置状态、案件—告警关系、通知、审计和后台任务由 Flyway 管理的关系表承载；Elasticsearch 继续承载事件、告警正文和实体风险检索。案件写入关系库后同步 ES 兼容镜像，历史 ES 案件按需惰性导入。
