...
- **我们**:已实现 ①(3 条单事件)+ ②(1 条窗口);④ 走 CEP(P1),③ 统计后置。
...
- **我们**:无。P2 先做 GeoIP(Logstash `geoip` filter 内建、近零成本),TI 用轻量本地缓存查表起步。
...
- **业界**:
  ...
  - verdict 回流闭环:结案打 TP/FP/duplicate → 按规则统计 FP 率 → FP>50% review(可减 50-80% 告警量不减覆盖)。
  - 痛点数据:62% SOC 告警被直接无视、55% 团队漏掉关键告警、84% 从业者倦怠。
- **我们**:当前"1 事件 1 告警"= 业界最不推荐形态。**P0**:suppression 去重(keyed state+TTL)+ `alert.risk_score` 排序 + 实体聚合(alert-service)。
...
- **我们**:无。P3 远期——ILM 长留存 + 留存策略文档化 + 合规 dashboard。
...
- **我们**:完整版**明确不做**(无 ML 基础设施、学习期长);最多对高频信号(认证失败率突增)做 Flink 统计基线 job(μ+3σ),限定误报可控场景。

#### ⑧ 调查 / 案件管理(Investigation & Case Management)

- **是什么**:分析师从"看到告警"到"搞清来龙去脉"到"结案"的工作台。
- **业界**:三线流转(open→ack→investigating→closed)、事件时间线、相关事件聚合;结案强制打 **verdict**(TP/FP/duplicate)并**回流调优规则**——这是误报闭环的关键纪律。
- **我们**:仅扁平告警 + `related_events`(窗口关联事件)。P1:`alert.status` / `alert.analyst_verdict` + Kibana 三线视图。
...
- **我们**:**明确不做**;告警→事件聚合放 alert-service(Spring Boot 占位工程)即可。

#### ⑩ 部署弹性(Deployment Resilience)

- **是什么**:SIEM 本身怎么部署、怎么扩(云 / SaaS / 自托管、高可用、灾备)。
- **业界**:本地 → 混合 → 云原生三代演进;云原生 SIEM 的 MTTD 比本地低 ~63%,混合架构采集成本低 ~41%。
- **我们**:单机 WSL2 + Docker Desktop(最简形态)。保持单机但**预留扩展点**(Kafka 分区、Logstash 多实例、Flink 并行),不提前做 HA。

**优先级全景**:

| 已具备 ✅ | P0 立即 | P1 近期 | P2/远期 | 明确不做 ❌ |
| --- | --- | --- | --- | --- |
| ECS 归一化、规则检测、窗口关联、Kibana 呈现 | ⑤ 风险告警(去重/评分)、③ 攻击链 CEP、⑧ 数据基础(risk_score/tags) | ⑧ 三线、② OCSF、④ GeoIP、⑩ 扩展点 | ⑥ 合规留存、④ TI 富化、⑦ 统计基线 | ⑦ UEBA/ML 完整版、⑨ SOAR、⑥ 报表包 |

> 应用顺序:**先把 ⑤(告警治理)和 ③ 的可靠性/关联补上,再谈覆盖广度**——别急着加规则。
