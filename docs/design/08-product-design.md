...
- **代码先落默认逻辑**:先实现本文档已定的默认阈值/口径(如失败率突升:本 1h 失败率 >5% 或 本 1h/前一 1h 环比 ≥2×,且本 1h 失败事件数 ≥20);同时作为**产品需求记录**——用户应能在系统设置中覆盖阈值或自定义规则(对齐业界 SIEM 的可调阈值 / 用户自定义规则能力)。
...
- **关键交互(字段级)**:
  - 健康指标:events1h / events24h / failRate / lastSeen(ES terms 聚合 `log.source_id`);failRate = 近 1h 该源 tags=_parsefailure 事件数 / 该源事件总数。
  - 失败率突升高亮:**本 1h 失败率 >5% 或 本 1h/前一 1h 环比 ≥2×**,且本 1h 失败事件数 **≥20** 才高亮(小样本不误高亮)。
  - 停采判定:lastSeen 超过 2× 该源正常到达间隔(默认 15min)无新事件 → 卡片异常。
  - 失败下钻:查看 _parsefailure 日志原文 → 决定「补模板(Story 02)/调采集/下线」;未知兜底桶 siem-events-raw 属 **P2 未落地**,现状取 siem-events-* 的 _parsefailure 事件(带原文)。
...
#### 5.7 调查台(远期)
- **职责**:案件聚合、关联时间线(远期;MVP 阶段以 Kibana 深度调查视图承接)
- **核心交互**:事件/告警聚合为案件、关联时间线(对标 Elastic Cases / QRadar Offense)
- **MVP**:远期,后置;前期以 Kibana 深度调查视图承接
...
| 解析失败率 | <5% | 近 1h 分源 _parsefailure 事件数 / 事件总数(§5.5) |
