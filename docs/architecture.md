...
7. 控制台写操作通过 Spring Security 鉴权；案件处置状态、SOAR 执行快照/步骤、用户会话、通知、审计和后台任务写入 PostgreSQL，Flyway 负责当前 V1-V8 迁移。
...
| SOAR | YAML Playbook、条件、白名单动作、审批、失败重试和步骤审计 | `/api/soar/**`、`soar_executions`/`soar_step_executions` |
