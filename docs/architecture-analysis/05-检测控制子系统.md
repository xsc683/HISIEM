...
> **读法导引** —— 本篇与 `06` 在本集里有特殊地位：**它们是各自领域唯一的厚度覆盖**。
> 检测控制在既有文档里只有 [`../design/managed-detection-runtime.md`](../design/managed-detection-runtime.md)（71 行），
> 而本篇覆盖 `detection-control`(30 main) + `detection-runtime`(16 main) + `detection-controller`(11 main) 共 57 个 main 文件。
...
4. **独立组合根**：`DetectionControllerApplication` 用**显式 mapper 白名单**——`@MapperScan(basePackageClasses = {ManagedDetectionMapper, DetectionArtifactMapper, DetectionRuntimeMapper, DetectionControllerMapper})`（`DetectionControllerApplication.java:19-25`），XML 侧配 `mybatis.mapper-locations=classpath*:mybatis/detection/*.xml`，且**没有** `sqlSessionFactoryRef`。
...
**当前规则集全部是认证相关规则**（`infra/rules/` 共 6 条：3 条 `single_event` + 1 条 `window` + 1 条 `cep` + 1 条 `baseline`）——这与 02 篇 §3.1 论断 1（`RuleRegistry` 注释「当前管道只有 SSH 认证失败事件」）一致，两处独立佐证了「当前检测域是 SSH 认证」这条事实。

**但不要把这句话读成编译能力的上限。** 编译器的类别集合是 `{single_event, window, cep, baseline}` 四类（`FlinkArtifactCompiler`），四类都已有实际规则在跑。受限于**规则集内容**的是检测覆盖，不是编译能力。
...
这与 V17 的四张表对应：`detection_job_group` + `rule_job_assignment` + `detection_runtime_manifest` + `rule_runtime_status`——**四张全在 V17**。（V16 建的是 `detection_rule`/`rule_revision`/`detection_plan`/`rule_deployment`/`rule_deployment_history`；V18 建的是 controller 租约相关表。另见本节 §7.1 的表-版本对照表。）
...
**论断 5：`detection-controller` 用显式 mapper 白名单，是**最窄**的扫描范围。**

```java
// DetectionControllerApplication.java:19-25
@MapperScan(
        basePackageClasses = {
