...
class AgentInvestigationControllerTest {
    ...
    void workspaceForwardsServerDerivedActorToService() throws Exception {
        ...
        SecurityContextHolder.getContext()
                .setAuthentication(new UsernamePasswordAuthenticationToken("analyst", "token"));
    ...
    void cancelUsesAuthenticatedOperator() throws Exception {
        AgentInvestigationService service = mock(AgentInvestigationService.class);
        when(service.cancel(eq(ID), eq("analyst")))
                .thenReturn(MAPPER.readTree("{\"status\":\"CANCELLED\"}"));
        SecurityContextHolder.getContext()
                .setAuthentication(new UsernamePasswordAuthenticationToken("analyst", "token"));
        AgentInvestigationController controller = new AgentInvestigationController(service);
    ...
    void alertLookupUsesAlertProviderAddressIdAndAuthenticatedActor() throws Exception {
        ...
        when(service.lookupForAlert("hisiem", "alert", "alert-doc-9", "analyst"))
                .thenReturn(
                        MAPPER.readTree(
                                "{\"active\":null,\"latest\":{\"investigation_id\":\"x\"}}"));
        SecurityContextHolder.getContext()
                .setAuthentication(new UsernamePasswordAuthenticationToken("analyst", "token"));
        AlertController controller =
                new AlertController(
                        mock(AlertService.class), mock(AgentLaunchService.class), service);

        assertEquals(
                "x",
                MAPPER.readTree(controller.agentInvestigation("alert-doc-9"))
                        .path("latest")
    ...
    void approveAndRejectRoutesSelectDecisionExplicitly() throws Exception {
        ...
        SecurityContextHolder.getContext()
                .setAuthentication(new UsernamePasswordAuthenticationToken("operator", "token"));
        AgentInvestigationController controller = new AgentInvestigationController(service);

        assertEquals(
                "APPROVED",
                MAPPER.readTree(controller.approveResponse("req-1", input))
                        .path("status")
                        .asText());
        assertEquals(
                "REJECTED",
                MAPPER.readTree(controller.rejectResponse("req-1", input)).path("status").asText());
