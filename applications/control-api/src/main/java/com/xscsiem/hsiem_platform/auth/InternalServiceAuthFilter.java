 ...
 *
 * <p>只由 {@code /api/internal/**} 专用 SecurityFilterChain 装配。故意不是 {@code @Component}: 否则 Spring Boot
 * 会把它注册成全局 Servlet Filter,绕过链的匹配范围。Bearer 与配置凭据做常量时间比较; 配置凭据为空、Bearer 缺失/错误或 {@code X-Tenant-ID}
 * 为空时必须全部拒绝(fail closed), 且不向调用方暴露失败细节。成功后设置 {@code ROLE_INTERNAL_SERVICE} 并使用服务端 Header 派生租户。
 */
