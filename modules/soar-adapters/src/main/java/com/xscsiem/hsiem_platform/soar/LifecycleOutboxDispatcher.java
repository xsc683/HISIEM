...
/**
 * Delivers the durable lifecycle outbox to Kafka. The database lease is the
 * authority for ownership; Kafka delivery is intentionally at-least-once.
 */
@Component
@ConditionalOnProperty(name = "app.soar.lifecycle-outbox-enabled", havingValue = "true", matchIfMissing = true)
public class LifecycleOutboxDispatcher {
    ...
    @Autowired
    public LifecycleOutboxDispatcher(ControlPlaneStore store, SoarKafkaProperties properties,
    ...
    public LifecycleOutboxDispatcher(ControlPlaneStore store, SoarKafkaProperties properties,
                                     MeterRegistry registry, boolean enabled, Duration lease,
                                     int batchSize, Producer<String, String> producer) {
        this.store = store;
    ...
    @Scheduled(initialDelayString = "${app.soar.lifecycle-outbox-initial-delay-ms:5000}",
    ...
    private void dispatchOne(Map<String, Object> item) {
        ...
        try {
            boolean completed = store.completeLifecycle(messageId, owner, true, null, Instant.now());
            if (!completed) {
                completionFailed.increment();
                LOG.warn("SOAR lifecycle outbox success completion was fenced messageId={}", messageId);
            } else {
                delivered.increment();
            }
        } catch (Exception e) {
            completionFailed.increment();
            LOG.error("SOAR lifecycle outbox success completion failed messageId={}: {}",
                    messageId, safe(e));
        }
    }
    ...
    private void completeFailure(Map<String, Object> item, String messageId, Exception cause) {
        ...
        try {
            boolean completed = store.completeLifecycle(messageId, owner, false, safe(cause), retryAt);
            if (!completed) {
                completionFailed.increment();
                LOG.warn("SOAR lifecycle outbox failure completion was fenced messageId={}", messageId);
            }
        } catch (Exception completionError) {
            completionFailed.increment();
            LOG.error("SOAR lifecycle outbox failure completion failed messageId={}: {}",
                    messageId, safe(completionError));
        }
        LOG.warn("SOAR lifecycle outbox delivery failed messageId={}, attempts={}, retryAt={}: {}",
                messageId, attempts, retryAt, safe(cause));
    }
    ...
    static String safe(Exception e) {
        ...
        String value = cause.getMessage() == null ? cause.getClass().getSimpleName() : cause.getMessage();
        value = value.replace("\u0000", "");
