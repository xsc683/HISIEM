...
/** 收敛进程重启后遗留的后台任务。 任务执行器是进程内线程池，无法在进程退出后继续持有执行上下文；恢复器将超过心跳窗口的 任务标记为失败，前端可以据此提示用户重试，而不是永久显示“进行中”。 */
@Component
@ConditionalOnProperty(
        name = "app.operations.runtime-enabled",
        havingValue = "true",
        matchIfMissing = true)
public class BackgroundTaskRecovery {
    ...
    public BackgroundTaskRecovery(
            TaskStore control, @Value("${app.tasks.stale-after:PT5M}") Duration staleAfter) {
        this.control = control;
        this.staleAfter =
                staleAfter.isNegative() || staleAfter.isZero() ? Duration.ofMinutes(5) : staleAfter;
    }
    ...
    @Scheduled(
            fixedDelayString = "${app.tasks.recovery-interval-ms:60000}",
        ...
        try {
            int recovered =
                    control.recoverStaleTasks(
                            Instant.now().minus(staleAfter), "服务重启或任务心跳超时，任务未能完成；请重新提交");
            if (recovered > 0) {
