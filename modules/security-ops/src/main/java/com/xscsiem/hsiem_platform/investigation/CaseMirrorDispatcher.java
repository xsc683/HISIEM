...
public class CaseMirrorDispatcher {
    ...
    @Scheduled(
            initialDelayString = "${app.cases.outbox-initial-delay-ms:5000}",
            fixedDelayString = "${app.cases.outbox-interval-ms:5000}")
    public void dispatch() {
        List<Map<String, Object>> batch =
                control.claimCaseMirrorBatch(owner, Instant.now().plus(lease), 50);
            ...
            try {
                ...
                if ("delete".equals(operation)) {
                    ...
                    Map<String, Object> document =
                            mapper.readValue(String.valueOf(item.get("payload")), MAP);
                    document.remove("_id");
                    document.remove("_control_version");
                    response =
                            elasticsearch.request(
                                    "PUT",
                                    "/siem-cases/_doc/" + caseId + "?refresh=false",
                ...
                LOG.warn(
                        "案件镜像 outbox 投递失败 caseId={},将于 {} 重试: {}", caseId, retryAt, e.getMessage());
            }
        }
    }
