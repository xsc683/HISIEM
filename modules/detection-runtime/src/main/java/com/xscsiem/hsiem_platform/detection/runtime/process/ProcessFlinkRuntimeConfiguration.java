...
public class ProcessFlinkRuntimeConfiguration {
    ...
    public DetectionArtifactBuilder detectionArtifactBuilder(
            JdbcTemplate jdbc, RuntimeManifestCodec codec,
            @Value("${app.detection.artifact-root:./data/detection-artifacts}") String artifactRoot,
            @Value("${app.detection.container-artifact-root:/opt/flink/detection-artifacts}") String containerRoot) {
        return new DetectionArtifactBuilder(jdbc, codec, Path.of(artifactRoot), containerRoot);
    }

    @Bean
    public ProcessFlinkRuntimeAdapter processFlinkRuntimeAdapter(
            CommandRunner commands, DetectionArtifactBuilder artifacts,
            DetectionJobNameCodec names, RuntimeManifestCodec codec,
            ...
            @Value("${app.detection.savepoint-root:file:///opt/flink/savepoints}") String savepointRoot,
        ...
        Set<String> whitelist = Arrays.stream(allowedClusters == null ? new String[0]
                        : allowedClusters.split(","))
                .map(String::trim).filter(value -> !value.isBlank()).collect(Collectors.toSet());
        return new ProcessFlinkRuntimeAdapter(commands, artifacts, names, codec, clusterId,
                containerName, jarPath, savepointRoot, commandTimeout, verifyPoll, whitelist);
    }
}
